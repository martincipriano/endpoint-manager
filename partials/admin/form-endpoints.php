	...
	<div class="rest-api-controls-row no-clear-button">
		...
		<div class="control-group">
			...
			<select id="type-filter" class="rest-api-filter-select">
				...
				<option value="static"><?php esc_html_e( 'Static', 'wpbuoy-endpoint-manager' ); ?></option>
				<option value="dynamic"><?php esc_html_e( 'Dynamic', 'wpbuoy-endpoint-manager' ); ?></option>
			</select>
		</div>
		...
		<div class="rest-api-route" data-methods="<?php echo esc_attr( implode( ',', $em_route_data['methods'] ) ); ?>" data-type="<?php echo esc_attr( $em_route_data['is_dynamic'] ? 'dynamic' : 'static' ); ?>">
			<div class="route-row">
				...
				<div class="route-info">
						...
						<?php if ( $em_route_data['is_restricted'] ) : ?>
							...
							$em_restricted_tooltip = $em_restricted_source // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound
								...
								? sprintf( __( 'Restricted by %s — may return 403 even when enabled.', 'wpbuoy-endpoint-manager' ), $em_restricted_source )
								: __( 'Restricted by its plugin or theme — may return 403 even when enabled.', 'wpbuoy-endpoint-manager' );
							?>
				...
				<button type="button"
					class="route-preview"
					disabled
					data-tooltip="<?php esc_attr_e( 'Dynamic Endpoint Preview (Pro)', 'wpbuoy-endpoint-manager' ); ?>"
					aria-label="<?php esc_attr_e( 'Dynamic Endpoint Preview (Pro)', 'wpbuoy-endpoint-manager' ); ?>">
				...
				<button type="button"
					class="route-preview"
					data-preview-url="<?php echo esc_url( $em_route_data['preview_url'] ); ?>"
