 ...
 */
function ramp_get_plugin_part( $slug, $name = '', $args = array() ) {
	if ( is_array( $name ) ) {
		$args = $name;
		$name = '';
	}
	$path = ltrim( $slug, '/' ) . ( $name !== '' ? '-' . $name : '' );
	$file = REST_API_MANAGER_PATH . 'partials/' . $path . '.php';
	if ( ! file_exists( $file ) ) {
		return;
	}
	if ( ! empty( $args ) ) {
 ...
 */
function ramp_return_plugin_part( $slug, $name = '', $args = array() ) {
	ob_start();
	ramp_get_plugin_part( $slug, $name, $args );
	return ob_get_clean();
}
